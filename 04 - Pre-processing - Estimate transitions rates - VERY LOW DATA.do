...
/******************************************************************************* 
Title: 			Estimate transitions rates from longitudinal data - VERY LOW DATA
...
//	Output model names	
	local job_exit_model 		= "job_exit_model_`country'_`years_avg'_final_VERY_LOW_DATA"
	local job_entry_model 		= "job_entry_model_`country'_`years_avg'_final_VERY_LOW_DATA"
...
// *hist entry_low_data
//
// twoway (hist entry_final, color(red)) (hist entry_low_data, color(green)) (hist transition_in, color(blue)) if  dens == 0, legend(label(1 "Full model") label(2 "Low Data") label(3 "Very Low Data"))	
...
// Note: We focus on those with low in-year density as an approximation of those who are currently out of work
	glm transition_in_ind c.spell_length i.wage_decile_ref c.age##c.age i.gender c.log_cod c.los_gap if dens <= 1, family(binomial) link(cloglog) cluster(id)
...
// 	Note: We focus on those with high in-year density as an approximation of those who are currently in work
	...
	generate transition_out_ind = entry_very_low_data * (1/adjustment)
...
estimates replay "job_ezit_model_MEX_10_final_LOW_DATA"
capture drop exit_low_data
predict exit_low_data
hist exit_low_data


twoway (hist exit_final, color(red)) (hist exit_low_data, color(green)) (hist exit_very_low_data, color(blue)) , legend(label(1 "Full model") label(2 "Low Data") label(3 "Very Low Data"))	
